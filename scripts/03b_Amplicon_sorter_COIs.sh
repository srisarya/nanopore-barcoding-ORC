...
outfolder_COIs="${output_base_dir}/${identifier_nopass}/COIs"

mkdir -p "${outfolder_COIs}"
...
consensus_file="${outfolder_COIs}/consensusfile.fasta"
if [ ! -f "${consensus_file}" ]; then
    echo "Error: output file '${consensus_file}' not made!! See $outfolder_COIs/$identifier_nosuff/results.txt for details'"
    exit 2
fi
...
{print}
' "${consensus_file}.tmp1" > "${identifier_nopass}_consensus_COIs.fasta"
